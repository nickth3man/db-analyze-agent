...
fn setup_env() { unsafe { env::set_var("OPENROUTER_API_KEY", "test-key"); } }
fn dbp() -> String { env::var("DATABASE_PATH").unwrap_or_else(|_| "../data/nba-data.duckdb".to_string()) }
fn has_db() -> bool { std::path::Path::new(&dbp()).exists() }

fn make_state() -> Option<AppState> {
    setup_env();
    if !has_db() { return None; }
    // Spawn on a fresh OS thread so Runtime::new() never nests inside an existing runtime.
    let dbp = dbp();
    std::thread::spawn(move || {
        tokio::runtime::Runtime::new().unwrap().block_on(async {
            let db = DbContext::new(&dbp).ok()?;
            build_state(db).await.ok()
        })
    }).join().unwrap()
}

fn with_state<F>(mut f: F) where F: FnMut(&AppState) {
    if let Some(s) = make_state() { f(&s); }
}
...
fn with_agent<F>(mut f: F) where F: FnMut(&Agent) {
    if let Some(a) = make_agent() { f(&a); }
}

// ---------------------------------------------------------------------------
// Agent::list_sessions (src/agent.rs:685-687)
// ---------------------------------------------------------------------------
#[test]
fn test_list_sessions_empty() {
    with_agent(|a| { assert!(a.list_sessions().is_empty()); });
}
...
fn test_export_session_markdown_nonexistent() {
    with_agent(|a| { assert!(a.export_session_markdown("nope").is_none()); });
}
...
fn test_reset_session_no_panic() {
    with_agent(|a| { a.reset_session("any"); });
}
...
fn test_build_router() {
    with_state(|s| { let _r = build_router(s.clone()); });
}
...
async fn test_health_handler() {
    let Some(s) = make_state() else { return; };
...
// ---------------------------------------------------------------------------
// sessions_handler (src/lib.rs:214-216)
// ---------------------------------------------------------------------------
#[tokio::test]
...
async fn test_export_handler() {
    ...
    let req = Request::builder()
        .uri("/api/export?session=nonexistent")
