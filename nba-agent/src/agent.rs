...
impl Agent {
    ...
    pub fn list_sessions(&self) -> Vec<(String, usize)> {
        self.sessions.read().iter().map(|(id, msgs)| (id.clone(), msgs.len())).collect()
    }
    ...
    pub fn append_markdown_table(md: &mut String, rows: &[serde_json::Value]) {
        ...
        let keys: Vec<&str> = rows[0].as_object()
            .map(|o| o.keys().map(|k| k.as_str()).collect())
            .unwrap_or_default();
        if keys.is_empty() { return; }

        md.push_str("| ");
        for k in &keys { md.push_str(&format!("{} | ", k)); }
        md.push('\n');
        md.push_str("|");
        for _ in &keys { md.push_str(" --- |"); }
        md.push('\n');
        for row in rows.iter().take(20) {
            md.push_str("| ");
            for k in &keys {
                let val = row.get(k).map(|v| {
                    if v.is_null() { "—".to_string() }
